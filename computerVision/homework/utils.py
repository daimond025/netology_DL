...
def calculate_mAP(det_boxes, det_labels, det_scores, true_boxes, true_labels, true_difficulties):
    ...
    for c in range(1, n_classes):
        ...
        for d in range(n_class_detections):
            ...
            find_ = torch.LongTensor(range(true_class_boxes.size(0))).to('cpu')
            true_class_images = true_class_images.to('cpu')
            this_image = this_image.to('cpu')

            # original_ind = torch.LongTensor(range(true_class_boxes.size(0)))[true_class_images == this_image][ind]
            original_ind = find_[true_class_images == this_image][ind].to(device)
...
def save_checkpoint(epoch, model, optimizer):
    ...
    filename = os.path.join(folder_curt,'model',  filename)
